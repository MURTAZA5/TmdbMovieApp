...
class MovieMultipleSearchAdapter(mContext : Context) : ListAdapter<Media, MovieMultipleSearchAdapter.MyViewHolder>(CHARACTER_COMPARATOR) {
    var context = mContext
    var selectedMovie = MutableLiveData<Media>()

    inner class MyViewHolder(private val binding: MoviesLayoutBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(response: Media?) {
            var path = AppConstants.LOAD_BACK_DROP_BASE_URL + response?.poster_path
