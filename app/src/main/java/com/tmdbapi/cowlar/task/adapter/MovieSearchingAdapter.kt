...
import com.tmdbapi.cowlar.task.databinding.ItemMovieSearchingBinding
import com.tmdbapi.cowlar.task.datamodels.MovieSearchResult

class MovieSearchingAdapter(mContext : Context) : ListAdapter<MovieSearchResult.MovieDetails, MovieSearchingAdapter.MyViewHolder>(CHARACTER_COMPARATOR) {
    var context = mContext
    var selectedMovie = MutableLiveData<MovieSearchResult.MovieDetails>()

    inner class MyViewHolder(private val binding: ItemMovieSearchingBinding) :
        ...
        fun bind(response: MovieSearchResult.MovieDetails?) {
            ...
            binding.tvMovieName.text = response?.title ?: "Movie Title"
            binding.tvMovieDetail.text = response?.overview ?: "Movie Details"

            Glide.with(context)
                .load(path)
                .centerCrop()
               .placeholder(R.drawable.placeholder) // Placeholder while loading
                .error(R.drawable.placeholder) // Error placeholder
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        return MyViewHolder(
            ItemMovieSearchingBinding.inflate(
